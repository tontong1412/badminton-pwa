...
const Manager = () => {
  ...
  const addManager = () => {
    ...
    request.post(`/tournament/add-manager`,
      {
        tournamentID: id,
        playerID,
      },
  ...
  const onRemoveManager = (playerID) => {
    request.post(`/tournament/remove-manager`,
      {
        playerID,
        tournamentID: id
      },
  ...
  return <>
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
      <div></div>
      <div>ผู้จัดการทั้งหมด: {tournament.managers.length} คน</div>
    </div>
    {
      tournament.managers.length > 0 ? tournament.managers.map(player => {
        return (
